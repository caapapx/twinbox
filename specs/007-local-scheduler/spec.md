...
`config/schedules.yaml` 不再引用 `twinbox-orchestrate`。文档写明 239 crontab 示例（08:30 / 02:00）。
...
- **FR-005**: Cron is external (239 crontab). Twinbox MUST NOT start an in-process timer or Unix-socket daemon.
...
- 239 上 Twinbox checkout 路径在 Sprint 0 探查后写入本目录 plan。
- 不实现 OpenClaw deploy/bridge。

## Out of Scope

- systemd 用户单元（239 无 systemd 用户会话假设）。
- 邮件写回与 SMTP。
