...
- **FR-006**: `latest_mail` / `todo` / `weekly` MUST expose `staleness`; when the pulse is missing they MUST run a full sync; when merely stale they MUST return the existing snapshot without IMAP refresh. MCP auto-recovery MUST pass through the query `account_id` (must not fan out to every account). Missing `weekly-brief-raw.json` MUST recover with `job=nightly-full`; latest/todo missing pulse keep default daytime-sync. Recovery that fails or returns `degraded` MUST NOT be presented as a complete weekly brief; a second read that is still missing MUST keep `ok=false` and the recovery hint. Explicit `twinbox_sync --job quick-refresh` may fetch + rebuild pulse without LLM analysis and exposes `analysis_generated_at`. `latest_mail` MUST return a compact card: `latest` plus at most 5 `threads` rows (no `fingerprint` / `query_terms` / `needs_attention` / `recent_activity` / `projections`). `todo` MUST return compact `needs_attention` rows without `fingerprint` / `query_terms`.
