...
| OpenClaw 内 Phase 2 onboarding 与上下文材料（对应 CLI：`twinbox openclaw …` / `twinbox context …`） | 插件：`twinbox_context_import_material` / `twinbox_onboarding_start` / `twinbox_onboarding_status` / `twinbox_onboarding_advance` / **`twinbox_onboarding_finish_routing_rules`**（routing_rules 阶段优先，一次执行内 rule add + advance） / `twinbox_onboarding_confirm_push` |
