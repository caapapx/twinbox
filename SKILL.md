...
**推送发往哪个会话：** 真源是 `runtime/push-subscriptions.json` 里每条订阅的 **`session_target`**。`twinbox_push_confirm_onboarding` 用插件解析顺序：**`TWINBOX_PUSH_SESSION_TARGET`** → **`OPENCLAW_SESSION_ID` / `OPENCLAW_SESSION`** → **`agent:twinbox:main`**。需要绑到**当前聊天**时用 **`twinbox_onboarding_confirm_push`** 并传 **`session_target`**，或宿主执行 **`twinbox push subscribe <SESSION> --daily on --weekly on`** / **`twinbox openclaw onboarding-confirm-push <SESSION> …`**。TTY `twinbox onboard openclaw` 里可在订阅前选默认或**自定义 session**。**digest 刷新时间**（非 bridge 每分钟 poll）：由 **`daily-refresh` / `weekly-refresh`** 的 cron 决定，默认见 `config/schedules.yaml`；覆盖在 **`runtime/context/schedule-overrides.yaml`**，命令 **`twinbox schedule list`** / **`twinbox schedule update daily-refresh '0 * * * *'`** 等；首次开 daily 时可能自动写入**每小时** refresh，可在 TTY 推送步骤或事后改掉。

**profile_setup 持久化细节：** CLI 参数 **`--profile-notes`** / **`--calibration-notes`** / **`--cc-downweight`** 映射到 `runtime/context/human-context.yaml`（`profile_notes` / `calibration`）加 `twinbox.json.preferences.cc_downweight.enabled`。Phase 2/3 **和 Phase 4** 的 **`context-pack.json`** 将这些暴露为 `human_context.onboarding_profile_notes` / `human_context.calibration_notes`。旧版 `manual-facts.yaml` / `manual-habits.yaml` / `instance-calibration-notes.md` / onboarding `profile_data.*` 在首次读取时迁移；之后统一文件为权威来源。无这些参数的阶段使用 `twinbox context upsert-fact` / `profile-set` 持久化文本。**`twinbox onboard openclaw`** 在 LLM 校验通过后可在 TTY 内多行粘贴画像/校准/参考文（单独一行 `.` 结束；不回显正文），可选 LLM 润色；跳过：`--skip-tty-context-bundle`。部署成功后默认继续在 TTY 完成 **routing_rules**（跳过 / 粘贴 rule JSON / 自然语言+LLM）与 **push_subscription**（需 host bridge timer）；仅对话完成：`--skip-tty-routing-push`。**material_import** 阶段先展示 `config/weekly-template.md`；用户要调整章节时转为 Markdown 并用 **`twinbox_context_import_material`**（插件）或 `twinbox context import-material FILE --intent template_hint`（或宿主上 **`twinbox context import-material --stdin --label STEM --intent …`**）导入，使后续 weekly digest 自动跟随 —— **文件存在的同一回合**完成，不要说"下一步再导入"。
