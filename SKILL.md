...
| OpenClaw 宿主接线（roots + `openclaw.json` + 复制 SKILL + 可选重启 Gateway）| `twinbox deploy openclaw --json`（`--dry-run`；`--no-restart`；`--no-env-sync`；`--strict`；可选 `--fragment` / `--no-fragment` 合并 `openclaw-skill/openclaw.fragment.json`） |
