...
| Weekly brief | `twinbox digest weekly --json`（人类可读模式为 Markdown，按默认 `config/weekly-template.md` 或最新 `template_hint` 的标题/章节顺序渲染；当前表示“当前周视图快照”，不是 daily 自动累计；稳定消费优先 `--json`） |
