...
Stages such as `profile_setup`, `material_import`, `routing_rules`, and `push_subscription` are **dialogue-first**: collect the user's answer in chat, but **the persisted stage only advances when** you run `twinbox onboarding next --json` on the Twinbox host. For **profile_setup**, add **`--profile-notes "…"`** for role/habits/preferences and **`--calibration-notes "…"`** for “this week’s focus / what to ignore / top priorities”; these are stored in `runtime/onboarding-state.json` as `profile_data.notes` / `profile_data.calibration`. Phase 2/3 **and Phase 4** **`context-pack.json`** pick them up as `human_context.onboarding_profile_notes` and, when `runtime/context/instance-calibration-notes.md` is absent, `human_context.calibration_notes` when the host runs phase loading (`python -m twinbox_core.context_builder phase2` / phase3, or the Phase 4 loading pipeline, e.g. `scripts/phase2_loading.sh` and the phase-4 load step). For other stages there is not yet an equivalent flag—use `twinbox context upsert-fact` / `profile-set` if you need durable prose.
