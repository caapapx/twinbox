...
description: >-
  ...
  rule_add plus advance. At push_subscription when the user confirms, prefer
  twinbox_push_confirm_onboarding (no session field) or twinbox_onboarding_confirm_push
  (session optional). Never say you will
...
- **At `push_subscription`:** when the user says **确认** (or agrees to daily/weekly), **same turn:** prefer **`twinbox_push_confirm_onboarding`** (only `daily`/`weekly` — **no session parameter**, cannot stall on session_target). Alternative: **`twinbox_onboarding_confirm_push`** with optional `session_target` (defaults as in plugin). Do **not** stall to “fetch session info”; call the tool and summarize JSON.
...
3. **Immediately after:** visible text with **`completed_stage`**, **`current_stage`**, next **`prompt`**. **Never** end with tool-only or with “下一步” narration and no tools.
...
- For onboarding after mailbox/LLM, prefer `twinbox_onboarding_start` / `twinbox_onboarding_status` / `twinbox_onboarding_advance`; **push_subscription** 优先 **`twinbox_push_confirm_onboarding`**（无 session 字段）；否则 `twinbox_onboarding_confirm_push`，避免仅依赖 `onboarding next` 的占位文案
