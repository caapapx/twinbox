...
| OpenClaw 安装总向导 V2（OpenClaw 风格显式步骤向导：Security、Quickstart/Manual、Mailbox、LLM、Twinbox tools integration、Apply setup + 更强 handoff；已有 Mailbox/LLM 值会先进入 `Existing config detected` / `Config handling`，LLM 覆盖输入顺序为 `API URL -> API key -> Model ID`） | `twinbox onboard openclaw-v2 --json`（验证入口；推荐人工体验评估时使用） |
