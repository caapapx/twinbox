...
| 命令 | 说明 |
| --- | --- |
| `twinbox daemon start` | 启动 daemon |
| `twinbox daemon stop` | 停止 |
| `twinbox daemon restart` | 重启 |
| `twinbox daemon status --json` | 机器可读状态（运行中含 `cache_stats` 等） |
...
│   ├── daemon.pid
│   └── daemon-supervisor.pid       # 旧版遗留；执行 `daemon stop` 后若存在会被清理
├── logs/
│   └── daemon.log
...
A: `**twinbox-orchestrate run`** 适合一次性或 cron 式跑完整/指定 phase 流水线。**daemon** 在本地提供 JSON-RPC 入口，便于宿主或工具按需触发子任务。日常「刷新 Phase 4」等仍可按需直接 orchestrate。
