...
def test_phase4_loading_includes_onboarding_profile_notes(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    state_root = tmp_path
    _write_mail_env(state_root)
        ...
        json.dumps(
            {
                ...
                "completed_stages": [],
                "profile_data": {"notes": "  phase4 onboarding note  "},
                "mailbox_config": {},
                "materials": [],
                "routing_rules": [],
                "push_enabled": False,
    ...
    assert hc["has_onboarding_profile_notes"] is True
    assert hc["onboarding_profile_notes"] == "phase4 onboarding note"


def test_phase4_loading_includes_calibration_notes(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    state_root = tmp_path
    _write_mail_env(state_root)
...
def test_phase4_loading_prefers_human_context_yaml_over_legacy_sources(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    state_root = tmp_path
    _write_mail_env(state_root)
    ...
    (phase2_dir / "persona-hypotheses.yaml").write_text('hypothesis: "x"\n', encoding="utf-8")
    (runtime_context / "manual-facts.yaml").write_text("facts:\n  - id: F1\n    content: legacy\n", encoding="utf-8")
    (runtime_context / "manual-habits.yaml").write_text("habits:\n  - id: H1\n    content: legacy habit\n", encoding="utf-8")
    (runtime_context / "instance-calibration-notes.md").write_text("legacy calibration\n", encoding="utf-8")
    (runtime_context / "human-context.yaml").write_text(
        yaml.safe_dump(
            {
                "profile_notes": "yaml note",
                "calibration": "yaml calibration",
                "facts": [{"id": "F2", "type": "role", "content": "yaml fact"}],
                "habits": [{"id": "H2", "type": "periodic", "content": "yaml habit"}],
            },
            allow_unicode=True,
            sort_keys=False,
        ),
        ...
        json.dumps(
            {
                "current_stage": "material_import",
                "completed_stages": ["profile_setup"],
                "profile_data": {"notes": "legacy note", "calibration": "legacy calibration"},
                "mailbox_config": {},
                "materials": [],
                "routing_rules": [],
                "push_enabled": False,
    ...
    context = run_phase4_loading(state_root, lookback_days=7, max_body_fetch=0, max_thread_candidates=10)
    hc = context["human_context"]
    assert hc["onboarding_profile_notes"] == "yaml note"
    assert hc["calibration_notes"] == "yaml calibration"
    assert "yaml fact" in hc["manual_facts_raw"]
    assert "yaml habit" in hc["manual_habits_raw"]
